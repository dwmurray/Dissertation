...
%While the next step may logically be MHD, we looked instead to the inclusion of protostellar jets. 

\section{Current State of the Field}
As a reminder to the reader, early work \citep{1977ApJ...214..488S,1992ApJ...396..631M,1997ApJ...476..750M} assumed that the pressure, 
which appears in the momentum equation, was given by $P = \rho c_s^2$ with $c_s = \textrm{const}$ or $P = \rho \vteqn^2$ with $\vteqn = \vteqn\ofr \propto \mathbf{r}^{\kappa_r}$, i.e., a prescribed function of $\mathbf r$, and $\kappa_r$ is typically chosen to be 1/2 to reflect Larson's law \citep{1981MNRAS.194..809L}.
...
\citet{2007ApJ...662..395N,2011ApJ...740...36N} found that the effect of protostellar outflows was important for driving turbulence.  \citet{2011ApJ...740..107C} and \citet{2012ApJ...747...22H} found that protostellar outflows enhanced the effectiveness of radiative feedback.  In addition, \citet{2015MNRAS.450.4035F} found that a combination of turbulence, jets, and magnetism is able to reproduced the observed low efficiency (to within a factor of four) of star formation in contrast to galactic scale simulations that rely on radiative and supernovae feedback.
...
\end{eqnarray}
where $\mathcal{T}$ is the angular distribution, $\mathcal{R}$ is the radial distribution, $r$ is the radial distance of the cell center from the $i$th sink particle.
...
v_{\rm jet} = f_{K} \sqrt{\frac{GM_*}{r_{\rm p*}}},
\end{equation}
where $f_{K}$ is the fractional percentage of the Keplerian velocity the jet reaches asymptotically, and $r_{\rm p*}$ is the radius of the accreting protostar.
We take $f_{K} = 1/3$, following \citet{2014MNRAS.439.3420M}.
To set $r_{\rm p*}$, we use two methods.
First, following \citet{2014ApJ...790..128F} we fix the protostellar radius at $r_{\rm p*} = 10\,R_{\odot}$.
...
In the center, we place a region of higher density, i.e., a clump, with a Gaussian density distribution, $\rho\ofr = \rho_m\exp(-\mathbf{r}^2/r_0^2)$,  with maximum density of $\rho_m=3\times 10^{-16}\,{\rm g\,cm^{-3}}$ and a characteristic radius $r_0=0.017$ pc.
...
\begin{figure}[ptb] %Figure 1 % Phil's test problem, 4 panels.
  \plotonebig{Jet/jet_panel.pdf}
  \caption[Jet -- Test Problem Panel]{Sequential plots of density structure perpendicular to the disk plane through the center of the sink particle. Columns from left to right show different times, from $t=0$ to $t=4820$ yrs, showing the effect of jet feedback on the test problem.}
...
We reproduce it here for convenience:
 \begin{eqnarray}
\label{eq:jet_deltaR}
\Delta r_{\rm p*} &=& 2 \frac{\Delta M_*}{M_*} \left( 1 - \frac{1 - f_k}{\alpha_g \beta} + \frac{1}{2} \frac{d\, \rm{log} \beta}{d\, \rm{log} M_*} \right) r_{\rm p*} - 2 \frac{\Delta t}{\alpha_g \beta} \nonumber\\
&&\times \left( \frac{r_{\rm p*}}{G M_*^2} \right) \left( L_{int} + L_{ion} - L_D \right) r,
\end{eqnarray}
where $\alpha_g = 3/(5 - n)$ describes the gravitational binding energy of a polytrope, and $\beta(n, M_*, r_{\rm p*})$ is the ratio of gas pressure to the total pressure for a polytrope of index $n$ and (proto)stellar mass $M_*$ and radius $r_{\rm p*}$ as defined above.
...
We precompute $\beta$ as a function of $n$, $M_*$, and $r_{\rm p*}$ and linearly interpolate with that table for specific values. The luminosities, $L_{\rm int}$, $L_{\rm ion}$, and $L_D$ are determined from equations (B6)-(B9) of O09 respectively.
...
If the radius of the star falls below $r_{\rm ms}$, where $r_{\rm ms}$ is the radius of the main sequence star of the same mass (from \citet{1967CaJPh..45.3429E},) we set $r_{\rm p*}=r_{\rm ms}$ and the state of the star is set to ``main sequence''.
...
The momentum of the jet is simply: $\vec{p}_{\rm jet} = m_{\rm jet} \times \vec{v}_{\rm jet}$; 
thus, the time rate of change in momentum due to the mass is: $\dot{\vec{p}}_{\rm jet} \approx \dot{m}_{\rm jet} \times \vec{v}_{\rm jet}$.
Writing this expression solely as a function of mass: $\dot{p}_{\rm jet} \propto m^{1/2} \times m^{1/2} = m_*$. 
Therefore the total integrated momentum: $\vec{p}_{\rm jet} \approx \frac{2}{3} m^{3/2}$.
So, if we do a comparison between the momentum of the jet from our threshold mass of $0.01$ to $0.1 \Msun$ vs $0.1$ to $1.0 \Msun$, we see that changing our threshold mass from $0.01 M_\odot$ to $0.1 M_\odot$ affects our result by $\approx 0.1 \%$.
...
We should note that this star formation criteria differs Chapter \ref{ch:hydro}, %from our previous work in \citet{2015ApJ...800...49L} and \citet{2017MNRAS.465.1316M}, 
which did not include these additional checks.
...
Figure \ref{fig:jet_jet_close_up} displays a thin slice $1\,{\rm pc}$ on a side centered on a star particle, showing the bubble inflated by the jet from the central star.
...
\begin{equation}
\dot{M}_* = f 4\pi \mathbf{r}^2 \rho(\mathbf{r}) \sqrt{\frac{GM_*(\mathbf{r},t)}{\mathbf{r}}},
...
First, the density around a collapsing region approaches an attractor solution, $\rhoatt$. % $\rho(r,t) \rightarrow \rho(r)$.
Second, the velocities inside the ``sphere of influence'' $\rs$, where $\rs$ is the radius at which the gas mass enclosed is roughly equal to the stellar mass, is controlled by gravity, i.e., $\vec{v}_r, \vec{v}_{\perp} \propto \sqrt{GM_*/\vec{r}}$.
...
Calculating the ratio of the stellar masses at a fixed time after star formation for the no jet vs jet runs, we find a ratio of $2.5$ and $2.6$ for the $16k^3$ and $32k^3$ resolutions.
...
\begin{figure}[htb]% Figure 5 % SFR Comparing the SFR for each changed fraction of m ejected via jet f
  ...
  \caption[Jet -- SFR Varying $f_{\rm jet}$]{Star formation efficiency (SFE) as a function of time since the formation of the first star, $\tts$ for 4 simulations, all with an effective resolution of $16k^3$. 
The jet simulations differ only in the fraction of accreted mass that they eject, the total momentum is the same across all jet simulations. 
The no jet (blue dashed) is the same simulation as the cyan dashed in Figure \ref{fig:jet_sfr} and the $0.3 {\rm \, jet}$ (solid green line) is the same as the thin solid red line in Figure \ref{fig:jet_sfr}. 
The dotted red line is $f_{\rm jet} = 0.1$, while the cyan dot-dashed line is $f_{\rm jet} = 0.5$. The effective resolution, jeans length and initial conditions are identical for all simulations in this plot. %For the jet runs, all three simulations have the same momentum deposition from jets. The sole difference, is in the amount of accreted mass that is ejected back into the GMC.
}
  \label{fig:jet_f_jet}
\end{figure}
...
Thus the (indirect) effect of the jet on accretion onto star particle (from turbulent driving, expulsion of accreting gas, etc) is $\approx 25 \%$, showing that the dynamical effect of jets on the mass accretion rate is minor.
...
In the next subsections we check the other predictions of MC15, $\rhoatt$ is an attractor solution, $\urb = \sqrt{GM_* /\vec{r}}$ is set by the protostar at $\vec{r}< \rs$, %where $\rs$ is the sphere of influence of the star particle (MC15), 
and the ``raw'' mass accretion rate --- without the $f_{\rm jet}$ correction ---  is the same in the cases with and without a jet.
...
\subsection{A Fixed Point Attractor for $\rho\ofrt$ Inside $\rs$} \label{sec:jet_run_density}

One of the more striking findings in MC15 and confirmed in the simulations of \citet{2017MNRAS.465.1316M} was that the run of density is independent of time for $\vec{r} < \rs$. %, where $r_*$ is the radius of the sphere which encloses a gas mass equal to $3 M_*$, i.e., the sphere of influence of the star.
...
In Figure \ref{fig:jet_ideal_density} we plot the averaged number density $n$ and mass density $\rho$ as a function of $\vec{r}$ for $1$ and $4 \, M_\odot$ sink particles.
The averages are over 36 and 30 particles respectively.
The plot confirms that $\rhoatt$ for $\rdrrs$, i.e. the density is already on an attractor solution and that profile persists well after formation.
We define the accretion disk radius $\rd$ as being the radius where the circular velocity ($\vphi$) is larger than $\vt$ and $\urb$.
...
This can be seen from Figure \ref{fig:jet_ideal_density}, where $\rho\ofr$ changes rapidly for $\vec{r} < 0.3 {\rm pc}$ before the star forms (see the dotted line).

\subsection{The Infall ($\urb$), Circular ($\vphi$) and Random Motion ($\vt$) Velocities with Protostellar Jets} \label{sec:jet_small_scale}

Figure \ref{fig:jet_velocity_profile} shows the infall velocity $\urb$ (blue triangles connected by a solid blue line), the random velocity $\vt$ (green dots connected by a green line), and the rotational velocity $\vphi$ (black plus signs) as a function of radius centered around a specific sink particle.
The red dashed line depicts $\sqrt{GM(<\vec{r})/\vec{r}}$, while the black horizontal line shows the sound speed $c_s = 0.265 \kms$.
We calculate each of these velocities in the same manner as Chapter \ref{ch:hydro},  %\citet{2017MNRAS.465.1316M}, 
with the following modification for the jet case: when calculating these otherwise spherical shell averaged quantities, we remove the cells that sit within the opening angle of the jet.
...
\begin{figure*}[htb] % Figure 7 % Velocity ID3_123 & ID4_124 for No Jet and Jet respect.
  ...
  \caption[Jet -- Velocity Profile Comparison]{The left panel is the run of velocity for a $\sim 3.0 \Msun$ sink particle with jet feedback. 
The right panel is the particle in the same location in a simulation without jet feedback, with mass $3.8 \Msun$. 
The sound speed is denoted by the black horizontal line while the infall velocity $\urb$ is given by the blue triangles, connected by a solid blue line. 
The green circles connected by a solid green line show $\vt$ while the black crosses show the rotational velocity $\vkep \equiv \sqrt{GM(< \vec{r})/\vec{r}}$. 
Note that both $\urb$ and $\vt$ increase with decreasing radius for $10^{-2} \pc < \vec{r} < 10^{-1}\pc$. }
...
That particle is roughly $3.8 \Msun$.
While we can not make a direct comparison of the magnitude of the velocities, due to the different masses, we can compare the trends in the velocities to the $\sqrt{GM(< \vec{r})/\vec{r}}$ velocity in each panel. 
We see similar behavior in both jet and no-jet simulations, with some expected differences.

First, what one immediately notices in each case (with and without jets), is that both $\urb$ and $\vt$ decrease with decreasing radius down to $\vec{r} \approx 0.1 \pc$ then increase with decreasing radius, until running into the accretion disk around the sink particle, at $\rd \approx 5\times 10^{-3} \pc$.
...
%Similar to \citet{2017MNRAS.465.1316M} we define the stellar sphere of influence as the radius ($r_*$) where the enclosed mass $M( <r, t)$ is three times the mass of the star.
 Both $\urb$ and $\vt$ increase inward of $\rs$. 
This was seen by \citet{2017MNRAS.465.1316M} for the no jet case.
...
However, the inclusion of jets does affect the ratio of the random motion velocity to the fiducial velocity $\sqrt{GM(<\vec{r})/\vec{r}}$.
In the case with jets, $\vt$ remains much closer to $\sqrt{GM(<\vec{r})/\vec{r}}$ for all radii larger than the disk radius as compared to that ratio for the no-jet simulation.
...
Even though the jets are boosting the random motion velocity, inside the stellar sphere of influence, $\urb$ still increases with decreasing radius. 
That is, the gravity of the star dominates the dynamics.% and thus the jets do not affect the general dynamics of the collapse inside $r_*$.
...
\subsection{Average Mass Accretion Rate $\langle \dot{M}\ofrt \rangle_{\rm stars}$ for Jet and No Jet Sink Particles} \label{subsec:jet_mdotstars}

The final prediction that we can check is that $\dot{M}\ofrt$ is independent of $\vec{r}$ for $\vec{r} < \rs$ and that the jet and no-jet cases have (roughly) the same $\dot{M}$.
In Figure \ref{fig:jet_mdot_shu}, we show the average mass accretion rate $\dot{M}$ as a function of $\vec{r}$ for $1$ and $4 \Msun$ sink particles comparing between the jet and no jet simulations.
...
We note that while the $\dot{M}$ profile is flat at small radii it does increase over time: the profile at $4 \Msun$ inside of $\rs$ is larger than that of the $1 \Msun$ profile for both jet and no jet simulations.
...
We also note that both the jet and no-jet case settle onto an average $\dot{M}$ that is roughly the same for the  $1$ and $4 \Msun$ sink particle case.  Hence the ``raw'' rate is the same in the jet and no-jet cases as implicitly required by equation (\ref{eq:jet_mass_ratio}).
%
...
\begin{equation}
\langle \vec{v}_{\rm jet} \rangle \equiv \vec{p}_{\rm jet} / M_* ,
\label{eq:jet_avg_jet_momentum}
\end{equation}
where $M_*$ is the total stellar mass and $\vec{p}_{\rm jet}$ is the total momentum ejected by all the jets up to the current time.
\citet{2000ApJ...545..364M} estimate (observationally) that $\langle \vec{v}_{\rm jet}\rangle \approx 40 \, {\rm km \, s^{-1}}$.
...
\begin{figure}[htb] % Figure 9 %
  ...
  \caption[Jet -- Jet Momentum over Time]{The average jet momentum per stellar mass as defined in equation (\ref{eq:jet_avg_jet_momentum}) as a function of time since the first sink particle forms ($\tts$). 
The solid green line depicts the $32k^3$ run while the dashed blue line depicts the $16k^3$ result. 
Both show an increase in $\langle \vec{v}_{\rm jet} \rangle$ with increasing time. 
This occurs because as time increases, the star particles gain mass, and thus have higher escape velocities and launch faster jets. 
The $16k^3$ run produces higher jet momenta per stellar mass because it forms fewer stars that are more massive, due to its lower resolution. Note however that the total stellar mass in the two runs is converged (see Figure \ref{fig:jet_sfr}).}
...
However, we note that the major difference between the no-jet and jet cases is the substantial amount of gas above $10\,{\rm km\,s}^{-1}$ and that the density of this gas is around 10x the mean density $3\times 10^{-22}\, {\rm g cm^{-3}}$.
In fact, the jet case has material moving above $100 \kms$.
...
The fact that $\urb$ increases inward for $\rrs$, while increasing outward for $\vec{r} > \rs$, combined with the time-independence of the density profile 
(once a density peak forms) implies that the mass accretion rate $\dot{M}\ofrt$ for individual stars is flat for $\rrs$ and increasing with radius for $\vec{r} > \rs$. 
This is demonstrated in Figure \ref{fig:jet_mdot_shu}, which also shows the contrasting behavior for an inside-out collapse model, that of \cite{1977ApJ...214..488S}.
...
In the left plot the rescaled no-jet case follows the jet case fairly well up to about an SFE of about 0.4. The rescaled no-jet case at this point re-scales the no-jet case for $M_*/M_{\rm tot}\approx 0.8$, where the accretion onto star particles is starved due to depletion of gas. 
...
Starting with uniform density in a box with length $16$ parsecs on a side, we drove turbulence until we reached a statistically steady state.
At that point, the density was no longer uniform.
We then turned on gravity and star formation.
We used AMR to follow collapsing regions down to an effective resolution of $32k^3$ which gave us a $\Delta x$ of $\sim 50 {\rm AU}$ at the finest level of refinement.
...
In this appendix we illustrate the effect of excising bi-cones aligned with the jet emitted by a protostar when calculating $\urb$, $\vt$ and $\vphi$. 
...
While the jets do not strongly affect the infall and rotational velocity of the bulk of the gas near the star particle, they do have a fairly strong effect on the random velocity, when averaged over spherical shells. 
A glance at Figure \ref{fig:jet_jet_close_up} shows why: the jet velocities are very large in the evacuated region around the jet axis. 
...
\begin{figure}[htb] % Figure 14 % The effect of subtracting the Jet cone
  ...
  \caption[Jet -- Effect of Subtracting the Jet Cone]{The run of velocity for a $M_* \approx 5.2 \Msun$ sink particle, 
averaged over spherical shells (left), averaged over the same shells but excising a bi-cone aligned with and having the same opening angle as the jet (middle) and excising a bi-cone with twice the jet opening angle (right). 
The sound speed is denoted by the black horizontal line while other lines denote the infall velocity $\urb$ (blue triangles connected by a solid blue line), the random velocity $\vt$ (green circles connected by a solid green line) and the rotational velocity $\vphi$ (black crosses); 
the dashed red line shows $\vkep \equiv \sqrt{GM \ofr / \vec{r}}$. 
Note that both $\urb$ and $\vt$ increase with decreasing radius for $10^{-2}\pc < \vec{r} <10^{-1}\pc$.}
...
All three panels follow the same convention: the infall velocity $\urb$ is depicted by the blue triangles, connected by a solid blue line,
the green circles connected by a solid green line show $\vt$ while the black crosses show the rotational velocity $\vphi$. The dashed red line shows $\vkep \equiv \sqrt{GM\ofr/\vec{r}}$.
Finally, the sound speed is denoted by the black horizontal line.

All three snapshots are of the same particle, of roughly $5.2 \Msun$, at the same time, from the same simulation.
In the top panel we calculate the various velocities averaged over spherical shells, i.e.,  we do not remove the jet bi-cone, in the center panel we remove the jet bi-cone, and in the bottom panel we remove a bi-cone with twice the jet opening angle.

The most dramatic change is in the turbulent velocity.
When we average over full spherical shells (top panel), the turbulent velocity remains roughly constant at all radii, with a slight increase at $\vec{r} >1$ pc.
In the two plots where we excise the jet bi-cone (or a bi-cone with twice the jet opening angle), we see similar behavior to runs where no jets were included; 
at large radii the random velocity $\vt$ decreases (by a factor of two) with decreasing radius, while at small radii ($\vec{r} \lesssim 0.1$ pc) $\vt$ increases with decreasing radius.

The effect of removing or not removing the jet bi-cone on $\urb$ and $\vphi$ is much smaller.
The infall velocity smooths out slightly when we excise the region around the jet, but the general dynamic of decreasing velocity with decreasing radius at large $\vec{r}$, and then inverting to increasing velocity with decreasing radius for $\vec{r} \lesssim 0.1$ pc is seen in all three panels.

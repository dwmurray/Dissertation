...
As we followed the collapse of this gas, we found that the character of the flow changes at two radii, the disk radius $\rd$, and the radius $\rs$ where the enclosed gas mass exceeds the stellar mass. 
Accretion starts at large scales and works inwards. 

If the equations are non-dimensionalized, two dimensionless variables appear, the Mach number ${\cal M}$ and the virial parameter $\alpha_{vir} \equiv 5 |\vteqn|^2 R / GM_{box}$, e.g., \citet{1984oup..book.....M}.
...
Most of the simulations described in this chapter use the adaptive mesh refinement code FLASH version 4.0.1 
...
Riemann solver and an unsplit solver \citep{2009ASPC..406..243L}. 
We have also extensively used the RAMSES code \citep{2002A&A...385..337T}, but unless explicitly stated otherwise, the results in this chapter come from FLASH simulations.
...
The resulting Mach number is ${\cal M} = 9$, i.e a turbulent velocity of $\vt = 2.37 \,{\rm km \, s^{-1}}$.

Stirring the initial turbulence using a fixed driving field is a
technique used by a number of workers in the field
\citep{2011ApJ...730...40P,2011ApJ...731...59C}. 
Other groups initialize the turbulence by initializing the velocity field with
Gaussian random perturbations having some assumed power spectrum
\citep{2014MNRAS.439.3420M,2015ApJ...809..187S}. 
While neither of the resulting velocity fields are generated the way the turbulence in the
...
We run for $\sim 700,000  \, {\rm yrs}$, about $0.18$ dynamical times, after the first star forms.
...
$1.25 \times 10^{-1} \pc$ which is sufficient to resolve the Jeans 
...
This condition states that if the Jeans length is less than the specified number of cell lengths, we need to refine to the next level. 
...
%
\begin{figure}[ptb] %Figure 1 %Zoomstar/quad2/ 498
%\begin{center}
\plotonebig{Hydro/f1.pdf}
\caption[HD -- Projected Density of the Simulation Box]{\label{fig:hydro_entire projection}Projection of the density along the z-axis of the entire simulation volume. 
The root grid is $128^3$ with up to 8 levels of refinement, giving an effective resolution of $32768^3$. 
...
\subsection{The Run of Infall ($\urb$), Circular ($\vphi$), and %Turbulent Velocity
 Random Motion ($\vt$) Velocities}% with Radius ($\vec{r}$) Before Star Particle Formation.}

Figure \ref{fig:hydro_Sphere_of_influence_quad2_well_before} shows the infall, $\urb$, circular, $\vphi$, and random motion, $\vt$, velocities as a function of  radius (top panel) and the density in a slice of the local volume 
...
We will compare $\vt$ to what  MC15 referred to as a turbulent velocity. Our current definition of $\vt$ is simply that of a random velocity. 
We are agnostic about whether or not $\vt$ characterizes an isotropic turbulent pressure; close examination of the velocity field indicates that the random motions are not isotropic on the scale of their distance from the density peak. 
It is also clear, however, that $\vt$ characterizes a Reynolds stress that does provide a net outward support against gravitational collapse. 
...
\ref{fig:hydro_Sphere_of_influence_quad2_well_before}, and the inward increase seen in later figures, shows that a substantial fraction of the potential energy released by the inflow goes into random motions. By energy conservation, this fraction is not available to the inflow, so that $\urb$ is smaller than it would be if the random motions were not absorbing some of the energy. 
This shows that there is an effective outward force on the infalling gas. 
%
...
\caption[HD -- Stellar Sphere of Influence I]{\label{fig:hydro_Sphere_of_influence_quad2_well_before}
The left plot shows the run of velocity with radius measured from the density peak; 
...
$\urb$ is given by the blue triangles, connected by a solid blue line.
The green circles connected by a solid green line show 
$\vt$ while the black crosses show the rotational velocity $\vphi$.
The red dashed line is the Keplerian velocity $\vkep \equiv \sqrt{GM\ofr /\vec{r}}$.
...
The right plot shows the density in a slice along the direction of the 
...
\caption[HD -- Stellar Sphere of Influence II]{\label{fig:hydro_Sphere_influence_quad2_prior_to_particle}
The left panel shows the run of velocity around the same density peak 
...
The color and linestyles are the same as in the left panel of Figure \ref{fig:hydro_Sphere_of_influence_quad2_well_before}.
The right panel again shows the density in a slice centered on the density peak.
The plotted arrows show the velocity in the plane of the slice.
The longest arrows correspond to roughly $2 \kms$.
...
mass of $\sim 0.7 \Msun$.
The radius of the sphere of influence (of the disk) is $\sim$0.02 pc. All three velocities, 
$\vt$, $\vt$, and $\vphi$, increase inward of $\rs$; the inflow is 
disrupted at $\vec{r} \sim0.015\pc$ a feature that we interpret as a shock, where 
the flow meets the nascent 
accretion disk, at which point $\vt$ also drops in magnitude. 
...
finite resolution are substantial inside $\vec{r} \approx 0.002\pc$, so features 
inside this radius are not reliable, and thus not plotted.}

\end{figure}
%

The infall velocity, $\urb$, and random motion ($\vt$) %turbulent
 velocity are similar in magnitude, and 
somewhat smaller than the Keplerian velocity, $\vkep = \sqrt{GM(<\vec{r}) /\vec{r}}$.
Note that $\urb$ is roughly equal to the sound speed while $\vt$ is supersonic.
...
However, $\vphi$ in the innermost regions (inside $0.01\pc$) is 
comparable to both $\vkep$ and $\vt$, showing that the innermost 
region is partially rotationally supported. The density slice, shown in the right 
...
We begin by developing an operational definition of $\rs$. 
We choose to define $\rs$ as the radius where the enclosed mass, $M(<\vec{r}, t)$ is three times the mass 
of the star, i.e., 
%
\be
3M_*(t) = M(<\rseqn(t), t)
...
when calculating $\rs$ and discuss how we define the disk mass in 
\ref{sec:hydro_Rotationally_supported_disks}. \label{footnote:disk_mass}}
In particular, the factor of 3 essentially means that the mass in gas is twice the mass of the central mass (star and disk) and implies that the gravitational acceleration of the gas is twice that of the central mass. 
This radius is where the dynamical effects of the gas begins to dominate the dynamical effects of the central mass.

Equations (\ref{eq:hydro_density}), (\ref{eq:hydro_infall_behavior}), and (\ref{eq:hydro_rstar def})
predict that the 
character of the solution should change at $\rs$ and that $\rs$ 
...
$\vt$ decreases with decreasing radius down to $\rs$ 
and then  increases with decreasing radius inside the sphere of influence. We see that $\vt$ reaches a minimum near $\vec{r}=\rs$.  
The inward decrease in $\vt\ofr$ is not monotonic near $0.2\pc$, probably due to a shock, 
as suggested by jumps in both the infall and random 
 velocities, and 
in the density, at $ \vec{r} \sim 0.02 \pc$.
This trend of increasing $\vt$ with decreasing radius inside $\rs$ is repeated in Figure %s \ref{fig:hydro_Sphere_influence_quad2_just_after} 
 \ref{fig:hydro_Sphere_influence_quad2_end_time}.

We don't see an increase in the infall velocity for $\vec{r} > \rs$ for 
this object because the star particles are forming about $1 \pc$ from 
the end of a filament, but we do see an increase in $\urb$ in other 
particles, see below.
%
...
\caption[HD -- Stellar Sphere of Influence III]{\label{fig:hydro_Sphere_influence_quad2_just_after}
The run of velocity (left panel) of \partA\ 24,000 years after 
star particle formation.
The color and linestyles are the same as in the left panel of Figure \ref{fig:hydro_Sphere_of_influence_quad2_well_before}.
This panel shows that the disk around the particle is rotationally supported for $\mathbf{r} \lesssim 5\times10^{-3}\pc$; inside that radius, 
...
or infall velocity.
The radius of the sphere of influence is $\rseqn \sim 0.06 \pc$.
The star mass is $2.47 \Msun$ and the disk mass is $0.58 \Msun$, 
...
The longest arrows correspond to nearly $3 \kms$.
...
\caption[HD -- Sphere of Influence IV]{\label{fig:hydro_Sphere_influence_quad2_end_time}
The left panel shows the run of velocity for \partB $\sim 100,000$ years after formation. 
The radius of the sphere of influence is $\sim 0.18 \pc$.  The stellar mass is $4.5 \Msun$ and the disk mass is $2.47 \Msun$ so the disk is $\sim 55\%$ of the
stellar mass. The right panel shows the density in a slice centered on \partB.}
\end{figure}
%
...
associated with $\rs$ increases over time. In particular, the global minimum 
...
The drop in $\urb$ at large radii in Figure 
...
This star is more isolated than particle A, and as a result $\urb$
% $\dot M(r)$
increases from $\vec{r}\gtrsim \rs$ out beyond $\vec{r} \approx 3\pc$. This is in
...
The behavior of $|\ur\ofr|$ at large radii is not set by the collapse dynamics, but rather by
...
In particular, we do not expect $|\ur\ofr|$ to be significant on scales larger than some moderate 
...
is more like $\vec{r} \approx 1\pc$ in our simulations.

\subsection{A Fixed Point Attractor for $\rho\ofrt$ Inside $\rs$}\label{sec:hydro_attractor}

One of the most striking findings of MC15 was that the run of density is 
independent of time for $\rrs$. 
...
(by a factor of 10 or more for $\vec{r}<0.1\pc$). 
We will show that in fact the density inside $\rd$ does change rather rapidly, after the star particle forms, but that for $\rdrrs$ the density does not change; see \S \ref{sec:hydro_ average profiles}

The mean power law slope of the density before the star forms (the blue dashed line in the figure) is $k_\rho \approx 1.9$, consistent within the star-to-star variations we see  with the range $k_\rho \approx 1.6 - 1.8$ from equation (\ref{eq:hydro_density}) for $\vec{r}>\rs$ (since in this case $\rs=0$).

%
\begin{figure}[htb] %Figure 6, density, showing that \rho(r,t)=\rho(r)
%quad2_65587_161_717
\plotone{Hydro/f6.pdf}
\caption[HD -- Run of Density]{\label{fig:hydro_quad2_run_of_density_double}The run of density for \partA. The dotted blue line is the 
density $\sim 40,000$ years before the star forms.
The solid green line is the run of density $\sim 540,000$ years after formation. 
The gap in time corresponds to the free-fall time at a radius of $0.24\pc$.
For $\vec{r} < 0.24 {\rm pc}$ the range of time spanned in the plot is more than a local free-fall time, yet the density does not vary significantly.
There are fluctuations in the density, e.g. the spikes around $\vec{r} \sim 0.1\pc$. 
...
In Figure \ref{fig:hydro_Mdot_vs_shu} we show the mass accretion rate $\dot{M}$ as a function of $\vec{r}$ around a star particle $\lp \tts > 0 \rp$ and from the corresponding density peak in which the star particle eventually formed $\lp \tts < 0 \rp$. 
This plot is taken from a RAMSES simulation. 
Before the star particle forms, $\dot{M}$ decreases inward at all radii. 

Following the establishment of the power law solution for the density, at $t=t_*$, a star particle forms and the $\dot{M}$ profiles flatten at small radii. 
An examination of the density profile (not plotted) reveals that $\rho \propto \mathbf{r}^{-3/2}$, while for $\tts=24$ kyrs, 
the gravitational force (and hence $\ureqn$) is dominated by the central mass for $\mathbf{r} \lesssim0.1\pc$, so that $\mathbf{v} \propto \mathbf{r}^{-1/2}$ out to that radius. We also note that while the $\dot{M}$ profile is flat, it does increase in time as shown by the difference between the $\tts = 24.6$ kyrs and $154$ kyrs curves. 
All this behavior agrees well with the prediction of Equation (\ref{eq:hydro_Mdot_behavior}).

At all times, the accretion rate is either nearly flat or increasing with radius, which is a natural result of the near balance between gravity and Reynolds stress support, as posited in the theory of MC15. 
We contrast this with an inside-out collapse model, which we exemplify using a \citet{1977ApJ...214..488S} solution (blue dashed line) obtained by directly integrating equations 11 and 12 of \citet{1977ApJ...214..488S} at a fixed time. 
The asymptotic behavior of $\dot{M}$ follows from Shu's equations 15 and 17;
...
density profile inside $\rs$ to an attractor 
...
developed rotationally dominated disk, at $\vec{r} \lesssim 0.05 \pc$ in 
Figure \ref{fig:hydro_Sphere_influence_quad2_just_after}. 

We define the outer edge of the accretion disk $\rd$ as the largest radius 
where $\vphi$ exceeds both $\urb$ and $\vt$, that is, where the 
...
figure, $\rd \approx 7\times10^{-3}\pc$. 
...
We note that the disks in our simulation have $\rd \sim 1,000$ AU. 
This is somewhat larger than the radii of the largest observed disks, e.g., \citet{1999AJ....117.1490P} find
$500\AU\lesssim \rd \lesssim 1000\AU$. Of course we are simulating massive
...
%
\be 
Q = \frac{\vphieqn \sqrt{\vteqn^2 + c_s^2}}{\pi G \mathbf{r} \Sigma}.
\label{eq:hydro_Toomre_Q}
\ee
%
In this expression $\Sigma$ is the gas surface density of the disk, $\vphieqn$ is the rotational velocity of the disk, and $\vteqn$ is the turbulent velocity in the disk. 
...
\ref{fig:hydro_Sphere_influence_quad2_end_time}.
For the region $3\times10^{-3} \lesssim \mathbf{r} \lesssim 6\times10^{-3} \pc$, $Q$ 
...
\caption[HD -- Toomre Q]{\label{fig:hydro_Toomre_Q_2B}The Toomre Q parameter for \partB, at the same time as shown in
...
dominated for $\mathbf{r}<2\times10^{-2}\pc$. 
For $3\times10^{-3} \lesssim \mathbf{r} \lesssim 6\times10^{-3} \pc$, $Q\lesssim 1$. 
...
that their $\vt$, $\urb$ and $\rho$ profiles are qualitatively similar
...
\caption[HD -- Run of Mass/Number Density]{\label{fig:hydro_density_avg}Number density as a function of radius at 10,000 and 100,000 
...
$\tts=-100,000\yrs$ terminates at $\mathbf{r}=0.02\pc$ because that corresponds 
...
For radii between $\rd \approx 0.02 \pc$ and $1 \pc$ the density profile does not 
...
said about the lack of evolution of $\rho\ofrt$ in the range $\rdeqn <\mathbf{r}\lesssim 0.1\pc$. 
...
Note that the density profile does increase for $\mathbf{r}<\rdeqn$; compare the green line 
(the $4 \Msun$ profile) versus the blue line (the $1 \Msun$ profile) in the 
...
all times. The change in the density for $\mathbf{r}<\rdeqn$ demonstrates that the density 
{\em can} evolve on the time scale of our simulations. Thus the fact that the 
density {\em does not} change for $\rdrrs$ between the two plots is striking. 
...
we fix $\rs$, and hence the velocity, $\rho$, and $\dot{M}$ profiles.  
...
$\rho\ofrt$ approaches a time independent function as soon as any 
...
$\vec{r}$, 10,000 and 100,000 years before star particle formation (left plot),
and for stellar mass $M_* = 1$ 
and 4 $M_{\odot}$ (right plot). The plots show 
that  $\rhoatt$ for $\rdrrs$, i.e., the density approaches the attractor solution,
...
It is important to note that the lack of change in the run of density is not due to the fact that we integrate for only a few tenths of a global free fall time. To emphasize this point, observe that the density at $\vec{r}<\rd$ {\em does} increase, while the density for $\rdrrs$ 
...
The reason for the increase of $\rho\ofrt$ with time for $\vec{r}<\rd$ is easy to understand: the gas is in a rotationally supported disk, which (as Figure \ref{fig:hydro_Toomre_Q_2B} shows) is marginally unstable. As the central stellar mass grows, the mass of the disk surrounding it will grow as well, in such a way that $Q\sim M_d/M_*$, where $M_d$ is the disk mass, is roughly constant. 

The fact that the density inside $\rd$ increases illustrates the general point that the relevant time scale for the run of density to change can be much shorter than the global dynamical time scale. 
If one had to wait for a global dynamical time, the density in the disk would not change over the entire course of our simulation, but Figure \ref{fig:hydro_density_avg} shows that the density in the disk does change over a tenth of the global dynamical (or free-fall) time. 
Thus the result that $\rhoatt$ for $\rdrrs$ is not a result of our short (relative to the global dynamical time) integration.

The one dimensional numerical models in MC15 also showed that
$\rhoatt$; MC15 find that the fixed point solution is
...
The net momentum of the converging flows will result in a net acceleration. % in the direction of one of the flows. 

The acceleration will be of the form:
\be
\mathbf{a} \approx \rho {\mathbf v}^2 / \Sigma_{\rm shock},
\ee

where, $\mathbf a$ is the acceleration due to the net ram pressure of the flow, $\rho$ is the upwind gas density, $\mathbf v$ is the velocity of the wind relative to the shock, and $\Sigma_{\rm shock}$ is the gas surface density of the shock. 
...
\ee
%
where $H = c_s^2 / \mathbf{a}$ is the scale height of the region of gas downstream of the shock that is in HSE.
...
for $\rho \propto \mathbf{r}^{-\alpha}$, the density PDF $\propto \rho^{-3/\alpha}$. 
...
\caption[HD -- Velocity vs. Density]{\label{fig:hydro_PDF_quad1}The probability distribution function of $n$. The black thin solid line
...
\caption[HD -- Sink Averaged Run of Velocity]{$\urb$, $\vt$, $\vphi$ and $\vkep$ as a function of $\vec{r}$ at 10,000 
(thin lines) and 100,000 (thick lines) years before the star particle forms 
(left plot) and when the star reaches 1 and 4$\Msun$ (right plot).  
The averages are over the same regions as those used in producing 
Figure \ref{fig:hydro_density_avg}. The infall $\urb$ and random $\vt$ velocities show 
the behavior predicted by the theory of adiabatic turbulent heating for times 
later than $-10,000$ years: 
at large radii, where $\urb$ is small, $\vt > \urb$ and $\vt$ decreases 
inward, but more slowly than in non-collapsing regions; $p\approx0.2$ 
rather than $p=0.5$. Inside $\rs$, 
where $\urb>\vt$ (or $|H|>\vt$ in the notation of 
\citet{2012ApJ...750L..31R}) $\vt$ {\em increases} towards $\urb$ 
as $\vec{r}$ decreases, with both increasing inward. \label{fig:hydro_velocity_avg}}
\end{figure*}
%

Figure \ref{fig:hydro_velocity_avg} shows the averaged $\urb$, $\vt$,  
$\vphi$, $\vkep$ as a function of $\vec{r}$ before the star particle forms 
...
particle forms and the same fixed masses ($1$ and $4 \Msun$) 
after star particle formation.  Here the dynamics of $\vt$ follow 
quantitatively the behavior of $\vt$ found in MC15. In MC15, $\vt$ 
scales with radius as $\mathbf{r}^{1/2}$ at very large $\vec{r}$, where self-gravity 
is not important. For example, at $t=100$ kyrs before 
the star particle forms, we find that $\vt\ofr\sim \mathbf{r}^{0.48}$, in line with
Larson's law, i.e., $\propto \mathbf{r}^{1/2}$. 
...
that the $\vt$ scaling has reversed itself at small 
radii ($\vec{r}\lesssim0.1\pc$) due to the accumulation of mass in a 
proto-disk; the gas in the disk deepens the potential well, but does 
not provide radial pressure support. The figure also shows that $\urb$ 
increases inward from $\vec{r}\approx0.1\pc$.

The reversal of the power-law form of both $\urb$ and 
$\vt$ as a function of radius tracks 
the position of $\rs$, as can be seen
comparing the lines for $t=-10$ kyrs in the left
plot with $M=1$ and $M=4 \Msun$ in the right plot. This confirms another
aspect of the MC15 solution --- that as $\rs$ moves outward with time the 
inflection point in $\urb$ and $\vt$ moves outward as well, as we found
earlier in \S \ref{sec:hydro_r_star}.

\begin{figure*}[htb] %Figure 12 total mass versus r
\plottwo{Hydro/f12a.pdf}{Hydro/f12b.pdf}
\caption[HD -- Run of Mass]{Mass of gas and stars as a function of $r$ at 10,000 (thin lines) 
...
the enclosed mass is convex, curving down as $\mathbf r$ decreases. By $t=-10,000$
...
The radial extent of this upward concavity is increased to $\mathbf{r} \approx 0.1\pc$ 
for $M_*=1$, and further to $\mathbf{r}\approx0.3\pc$ by the time the stellar mass reaches 
$M_*=4\Msun$. The position of $\rseqn$ can be inferred by the position 
in the curves where the concave portion of the curve meets the linear portion.  
The concave regions are dominated by a central mass and hence are inside of $\rseqn$.  

The growth of the central mass forces $\rseqn \oft$ outward because $\rho\ofr$ 
is independent of time, and hence the gas mass at small radii remains fixed, while
$M_*(t)$ grows. 

Returning to the velocities, the fact that $\vteqn \ofr \sim \mathbf{r}^{0.48}$ for $t=-100$ kyrs in the left plot of Figure \ref{fig:hydro_velocity_avg} shows that the 
turbulence in the 
...
However, the flattening and reversal of $\vteqn \ofr$ 
...
behavior of $\vteqn\ofr$ as the combined result of compressional heating and 
...
%
At early times, $\urb$ is between $\vkep/3$ and $\vkep$, except 
at $\vec{r}\gtrsim 1$ pc, where the clump fades into the ambient 
molecular cloud.  These high ratios of $\urb/\vkep$ show that 
hydrostatic equilibrium is not a valid description of the star forming 
regions at any time. 

In fact, these plots show that $\urb$ is of order $\vkep/3$ or larger at all 
times for $\rd\lesssim \vec{r}\lesssim 1\pc$.
...
At small radii, the fact that $\rho\ofrt =\rho\ofr \sim \mathbf{r}^{-3/2}$ for $\rrs$, 
combined with the fact that $|\ur \ofrt|\sim \mathbf{r}^{-1/2}$, ensures that 
$\dot{M}\ofrt =\dot{M} \oft$, i.e., the mass accretion rate is independent of 
radius for $\mathbf{r} < \rseqn$. 
...
evolving toward the attractor solution. But for later times $\dot{M} \ofr$ 
...
which we define as $\vphieqn^2/\mathbf{r}$ (solid blue line), Reynolds stress
plus thermal pressure support $\rho^{-1}d\mathbf{P}/d\mathbf{r} = \rho^{-1}d\rho
(\vteqn^2/2+c_s^2)/d\mathbf{r}$ (dashed line), and total pressure support
$\rho^{-1}d\mathbf{P}/d\mathbf{r} + \vphieqn^2/r$ (thick red line). 
We have scaled these quantities to the local gravitational acceleration, $g=GM(<\mathbf{r})/\mathbf{r}^2$.


Inside of $\mathbf{r}\approx 0.01$ pc, the gas settles into a rotationally supported 
...
\caption[HD -- Ratio of Pressure and Gravity]{The ratio of rotational (solid line) support, Reynolds stress and gas 
(dashed line) pressure support to the local gravitational acceleration 
$g = GM(<\mathbf{r})/\mathbf{r}^2$, as a function of $\mathbf{r}$ when the star reaches 4$\Msun$. \label{fig:hydro_dPdr_avg}}
\end{figure}

The rotationally supported disks in our simulations are, on average, 
roughly marginally stable,  
as seen in Figure \ref{fig:hydro_Q_avg}, which should be compared with Fig. 3 in \citet{2010ApJ...708.1585K}.  For $0.007\pc\lesssim \mathbf{r} \lesssim 0.02\pc$, 
...
star, while at other times the disk is stable, building up material to approach marginal stability. 
For $\mathbf{r}\gtrsim 0.02$ pc, $Q$ rises, though the interpretation of 
...
\caption[HD -- Toomre Q Average]{The average (over many disks) of Toomre $Q$ as a function of $\mathbf{r}$ when  $M_*\approx 4\Msun$. \label{fig:hydro_Q_avg}}
\end{figure}
...
produce the total SFE history as follows. 
First, we analyze the simulations to 
find the earliest time at which a star particle formed, which we define as $t_*$.  
We then look at all of the octant simulations to find the earliest time at which a simulation 
ended or $t_{\rm end}$, which defines the time over which all our simulations 
have data. 
Because each snapshot for each simulation is taken at different times, we define a number of times at fixed intervals between $t_*$ and 
...
that at $\mathbf{r}\approx1\pc$ the density has already settled onto its time-independent form, while 
Figure \ref{fig:hydro_velocity_avg} shows that the infall velocity is scaling as $\mathbf{r}^{-1/2}$ for 
$\mathbf{r}\lesssim 0.3-0.7\pc$, with the smaller value corresponding to the time of star particle 
formation, and the larger value to times for which $M_*\approx 1\Msun$. As long as
a simulation resolves this radius (which corresponds roughly to $\rs$), it will recover the
$M_*\sim t^2$ scaling. 
...
infall velocity is larger than $\sqrt{GM_*/\mathbf{r}}$; it takes time before the Reynolds stress can slow the infall to the steady state value given by equation 
...
\caption[HD -- Mass Accretion Rate over Time]{The total mass in stars plotted as a function of time since the first star 
...
The Keplerian or outer disk radius, $\rd$, i.e., the radius at which the gas becomes rotationally supported, and 
the radius, $\rs$, of the stellar sphere of influence, the smallest radius at 
which the gravity of the gas dominates the gravity from the star and disk. 
The existence and significance of $\rd$ has been known since the time of 
Kant and Laplace; the recognition that $\rs$ plays a role in star formation 
is recent, so we concentrate on the effect of $\rs$ on the dynamics in 
what follows. The value of $\rs$ increases monotonically with time, since 
the stellar mass increases monotonically, while $\rd$ may vary in or out 
...
variation of $\vt$ with radius; for $\rdrrs$, the 
random motion velocity is a decreasing function of radius 
$\vteqn\ofr \sim \mathbf{r}^{p}$ with $p\approx -1/2$, while for $\rseqn< \mathbf{r}$, it is
an increasing function of $\mathbf{r}$, scaling like $\mathbf{r}^{0.2}$.
Similarly, the infall velocity $\urb \sim \vec{r}^{-1/2}$ for $\rdrrs$,  
while for $\rs<\vec{r}$ it is flat or even increasing outward (as in the top 
...
or infall times;  for $\rdrrs$, $\rhoatt$. 
This is illustrated by Figures \ref{fig:hydro_quad2_run_of_density_double} and \ref{fig:hydro_density_avg}. 
...
The fact that $\rho\ofrt=\rho\ofr \sim \mathbf{r}^{-3/2}$ for $\rrs$, combined
with the fact that $|\ur\ofrt|\sim \mathbf{r}^{-1/2}$, ensures that $\dot{M}\ofrt=\dot{M}(t)$, i.e., the mass accretion rate is independent of
radius for $\rrs$ (see Figure
\ref{fig:hydro_Mdot_vs_shu}). 

Since $\urb$ increases with stellar mass and hence with time where $\rrs$ 
...
In our simulations,
the support from random motions slows the rate of infall, so that $\urb$ is significantly smaller
then the free-fall velocity, but large enough to maintain $\vt$ at a sufficient level
...
show that for $\mathbf{r} > \mathbf{r}_*$ the bulk of the potential energy goes into random motion, 
and thence into shocks. Inside $\rs$, but outside of the disk, the potential energy 
...
($\mathbf{r}\gtrsim 1\pc$) the turbulent kinetic energy can exceed the potential
energy: on the scale of our box, the ratio 
$\vteqn /\sqrt{GM_{\rm GMC}/r_{\rm GMC}}\approx \alpha^{1/2}_{\rm vir}\approx 1$,  
...
of the box.  This ratio scales as $\sim 1/\sqrt{\mathbf{r}}$, so at 
$\mathbf{r}\approx 1\pc$ the ratio is $\sim\sqrt{8}$.
...
compression (for $\vec{r}>\rs$) or dilation (for $\rrs$), and on the
...
law is $\vteqn\sim \mathbf{r}^p$ with $p\approx 0.4-0.5$. It emerges naturally from the 
...
$\vteqn\sim \mathbf{r}^p$ with decreasing radius is slowed for $\mathbf{r}_*< \mathbf{r} \lesssim 1\pc$. 
Least squares fits to $\vt \ofr$ for over this range of radii result in 
exponents between $0.1\lesssim p\lesssim 0.4$, with an average around 
$p\approx 0.2$, in fair agreement with the prediction of equation (\ref{eq:hydro_infall_behavior}). 
Inside of $\rs$, we find $p=-1/2$, as predicted by equation (\ref{eq:hydro_infall_behavior}), representing a reversal of Larson's law. 
...
and that, both before and after a star particle forms, $\dot{M}\ofrt$ is larger at 
large $\vec{r}$ than it is at small $\vec{r}$; in other words, the collapse proceeds in an outside-in manner.
...
\ref{fig:hydro_velocity_avg}, which show that $|\ur\ofr| \sim (1/3) \vkep\ofr$ out to $ \vec{r} \sim 1\pc$ or
...
of the gravity term for $\vec{r}>\rd$}

Figure \ref{fig:hydro_dPdr_avg} shows that the 
acceleration due to the pressure gradient is comparable to the 
acceleration of gravity for $\rdrrs$. 
Thus Reynolds stresses slow the infall compared to the free-fall rate, i.e., $|\ur\ofrt| < \sqrt{GM(\vec{r})/\vec{r}}$.  
At small radii ($\vec{r} < \rd$), the rotational support becomes important and 
the support from $\vt$ becomes much smaller than the radial component
of gravity. %, but comparable to the vertical component in the disk. %Remove due to Phils comment.
...
line width-size relations than the classical Larson result, i.e., $\vt \ofr \sim \mathbf{r}^p$ 
...
collapsing flow at $\mathbf{r}> \mathbf{r}_*$. 
...
$D=1.7\kpc$ see infall $\urb=0.1-0.6\kms$, $\sigma_T\sim 0.6-2\kms$ at  
$\vec{r} \approx 0.1\pc$, $n\sim 10^5-10^6$. Other examples include \citet{2012ApJ...746..174R,2015A&A...573A.119R} and 
\citet{2013A&A...555A.112P}. 

Infall is also seen on larger scales, $\vec{r} \approx1\pc$, by \citet{2016A&A...585A.149W}, who observe $\urb$ in the range $0.3-3\kms$, 
corresponding to a fraction of the free-fall velocity ($1.4$ the Keplerian velocity) of $0.03-0.3$. 
In words, the gas at $\vec{r}=1\pc$ is not in hydrostatic equilibrium, nor is it in free-fall. 
The turbulent velocity in the same clumps at the same radii is comparable or slightly in excess of the infall velocity, $\vt \approx 1.0-2.3\kms$.

\citet{1986ApJ...304..501H}, \citet{2008ApJ...684.1273K}, and 
\citet{2011A&A...530A..53K} see infall at three different radii, 
$\vec{r}\approx 0.5\pc$, $\vec{r}\approx 0.3\pc$, and $\vec{r}\approx 0.03\pc$ using 
different molecular tracers in the same object, G10.6-0.4. The infall velocity
is large at large $\vec{r}$, small at $\vec{r}\approx 0.3\pc$, and large again at $\vec{r}\approx 0.03\pc$. 
...
that two length scales emerge from the process of star formation, $\rs$ and $\rd$, and demonstrate
that these length scales are clearly associated with physical effects. 
In particular, the character of the 
solution changes at $\rs(t)$, inside of which (but outside $\rd$) $\urb$ and  $\vt$ are both
$\propto \mathbf{r}^{-1/2}$; outside of $\rs$, $\vt \sim \mathbf{r}^{p}$ (with $p\approx0.2$), while $\urb$ 
...
$\rs(t) \propto M_*^{2/3}(t)$.
The disk radius, $\rd$, also changes as a function of time as a result of the advection and transport of angular momentum from large scales to small scales (and vice versa).

We also found that the density profile evolves
to a fixed attractor, $\rhoatt$ in line with the results of MC15 and 
...
We note, as did MC15, that observations of massive star forming regions also find $\vteqn \propto \mathbf{r}^p$ 
with $ p \sim 0.2-0.3$, and that at small radii or high density, $\vteqn$  
...
due to the pressure gradient is comparable to that due to gravity at all $\vec{r}>\rd$. 
...
In particular, we find the development of rotationally support disks at $\rd \sim 0.01$ pc.  
...
scales ($\vec{r}\sim 1\pc$) with large initial infall velocities.  In addition, we find that $\vt$ 
...
$\mathbf{r}^{-3/2}$ for $\mathbf{r}<\mathbf{r}_*$ and $\ureqn$ scales with the Keplerian velocity guarantees that $\dot{M}$ is 
...
\section{Appendix} 
Prior to proceeding directly into the inclusion of protostellar jets, we wish to note several things.
...
\caption[HD -- Normalized $\dot{M}$ and Normalized Solid Angle]{Histogram of the cumulative normalized $\dot{M}$ (left) and cumulative normalized solid angle (right) as a function of normalized density for $r=0.05$ and $0.5\pc$ when the star reaches 1 and 4 $M_{\odot}$. \label{fig:hydro_Accretion_avg}}
\end{figure*}
...
\caption[HD -- Non-filamentary Accretion]{Histogram of the cumulative normalized averaged $\dot{M}$ as a function of the cumulative normalized averaged solid angle for $r=0.05$ and $0.5\pc$ when the star reaches 1 and 4 
...
\caption[HD -- Comparison of Particle Formation Checks]{The run of velocity for a particle formed in a simulation using FLASH's built in particle formation checks (as well as the density condition proposed by \citet{2011ApJ...730...40P}). 
...
\caption[HD -- Radial vs Lateral Turbulence]{The radial $v_{\rm T,\, r}$ (blue solid), lateral 
...
\caption[HD -- Convergence in Jeans Length]{Plot of $M_*(t-t_*)$ for Ramses runs with $N_J = 32$ (blue), $N_J = 8$ (green), $N_J = 4$ (red). At the end of the $N_J = 32$ run, the total stellar mass was $M_*\approx 240M_\odot$. \label{fig:hydro_Convergence}}
\end{figure}

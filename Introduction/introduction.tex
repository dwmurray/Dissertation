...
It was rapidly realized that the molecular gas, which had a total mass of about $1.6\times10^9\Msun$ \citep{1993AIPC..278..267D,2017ApJ...834...57M} 
...
The total mass of the latter is about $7\times10^9\Msun$. 
...
The velocity dispersion in the WNM is $\Delta v \simeq 24 \kms$ \citep[p. 49]{2009ARA&A..47...27K}, corresponding to the line of sight velocity dispersion of $\vec{\sigma}_{\rm los} \approx 10 \kms$. This is comparable to the thermal line width.
...
%
\be
\left({{\mathbf \sigma}\over \kms}\right) = 1.10\left({L\over \pc}\right)^{0.38}, 
\ee
%
...
Simulations of supersonic turbulence find a relation of the form $\vec{\sigma}\sim L^{1/2}$,
where $\vec{\sigma}$ is the dispersion velocity, and L is the length scale on which the velocity dispersion is measured. 
...
%
\be
\left({{\mathbf \sigma} \over \kms}\right) = 0.42\left({M\over M_\odot}\right)^{0.20}.
\ee
%
...
%
\be
\left({{\mathbf \sigma}\over \kms}\right) = 0.72\left({R\over \pc}\right)^{0.21},
\ee
%
to be compared with their result for low mass star forming cores
%
\be
\left({{\mathbf \sigma} \over \kms}\right) = 0.64\left({R\over \pc}\right)^{0.52}.
\ee
%
...
The most common source of ionization in the Galaxy is radiation from massive ($\sim 40\Msun$) stars, which, as we saw above, are less then about 4 Myrs old. Hence free-free emission is a tracer of star formation.
...
A second measure of efficiency is how much gas turns into stars in a free-fall time. If we imagine an isolated cloud of gas with constant density, $\rho(\vec{r}) \rightarrow \rho$, we can derive the amount of time required for the cloud of gas to collapse to a single point.
...
%
\be
\alpha_{\rm vir}\equiv 5{|\vec{v}|^2 R\over GM},
\ee
%
where $M$ is the mass of the cloud and $\vec{v}$ is the line of sight velocity. 
...
\section{Star Formation on the Galactic Scale, The Kennicutt-Schmidt Law}
...
Beginning in the late 1960's, star formation theory was the idea that clumps destined to form stars started in hydrostatic equilibrium (HSE). The gas in a cloud was assumed to be supported against its self gravity via thermal pressure support. 
...
He also finds that the density approaches a constant at small radii, with the result that $\dot{M} \propto \vec{r}^3 $. 
...
In other words, the mass accretion at large radii does not depend upon the radius. 
Note that this asymptotic result applies well inside the outer boundary.
...
One of the issues with the model was the assumption that the flow at large radii be directed inwards at velocity $ \vec{v} \rightarrow 3c_s$ as $\vec{r} \rightarrow \infty$. 
...
\citet{1977ApJ...214..488S} predicted a maximum accretion rate of $\sim 2\times10^{-6} \Msun\,\yr^{-1}$.

\subsection{\citet{1992ApJ...396..631M}}%{Theory of Turbulent Pressure Support}
Shu's predicted maximum  accretion rate is too small to explain the origin of massive ($M_*\sim50-100\Msun$) O stars, which have lifetimes $\lesssim 4\times10^6\yrs$, but would take 50 Myrs to grow, according to his model.
...
%
\be
{\mathbf \sigma}^2 = {\mathbf \sigma}_T^2 + {\mathbf \sigma}_{NT}^2, 
\ee
%
where ${\mathbf \sigma}_T$ is the thermal velocity and ${\mathbf \sigma}_NT$ is the turbulent or nonthermal velocity. 

For radii where the non-thermal velocity dominates the line-width, 
%
\be
\dot M_* = m_o \vteqn^3/G,
\ee
%
where $\vteqn$ is the velocity inferred from the observed linewidth. 
...
They also assumed that the turbulence is static and unaffected by the collapse, i.e. $\vt\ofrt \rightarrow \vt\ofr$.  
...
are referred to as inside-out collapse models; the collapse starts at small radii (formally at $\vec{r}=0$ in the analytic models) 
and works its way outward, at the assumed propagation speed ($c_s$ or $\vt\ofr$). 
At any given time, the infall velocity and mass accretion rate both decrease with increasing radius $\vec{r}$. 
The analytic models assume the existence of a self-similarity variable $x = r/vt$, where $v=c_s$ in 
\citet{1977ApJ...214..488S} or the turbulent velocity $\vt\ofr$ in \citet{1992ApJ...396..631M,1997ApJ...476..750M,2003ApJ...585..850M}.  
...
\citet{2003ApJ...585..850M} present evidence that massive-star forming regions are supersonically turbulent, 
...
In addition, they also find in their Figure 4, that the density goes to $\rho \propto \vec{r}^{-3/2}$.
...
Larson's law, $\vt\ofr\sim \vec{r}^{1/2}$ \citep{1981MNRAS.194..809L}. 
...
%
\be
\frac{\partial \vteqn}{\partial t} + \ureqn \frac{\partial \vteqn}{\partial r} 
+ \left( 1 + \eta \frac{\vteqn}{\ureqn} \right) \frac{\vteqn \ureqn}{r} = 0
\label{eq:hydro_Robertson}
\ee
%
The first two terms are the Lagrangian derivative, and $\ureqn$ is the radial infall velocity. The first term in the brackets 
...
\begin{itemize}
\item The gravity of the newly formed star introduces a physical scale into the problem, which MC15 called the stellar sphere of influence, $\rs$. This is an idea familiar from galactic dynamics. 
The radius $\rs$ is where the local dynamics transitions from being 
dominated by the mass of the gas to being dominated by the mass of the star. 
As a result, the character of the solution, in particular that of the velocity, differs dramatically between $\rrs$ and $\vec{r} > \rs$. 
...
\rho({\mathbf r},t)=
\begin{dcases}
\rho(r_0) \lp {\frac{\mathbf r}{r_0}}\right)^{-3/2}, & \rrseqn \\
\rho(r_0,t)\lp {\frac{\mathbf r}{r_0}}\right)^{-k_\rho}, \ k_\rho\approx1.6-1.8 & {\mathbf r} > \rseqn\oft.
\end{dcases}
\label{eq:hydro_density}
\ee
%
where $r_0$ is some fiducial radius. 
\item The existence of $\rs$ implies that the infall and turbulent velocities have different scaling for $\rrs$ and $\vec{r}>\rs$.  In particular, MC15 showed
%
\be
\ureqn\ofrteqn, \vteqn\ofrteqn \propto
\begin{dcases}
\sqrt{\frac{GM_*\oft}{{\mathbf r}}} \sim {\mathbf r}^{-1/2} & \rrseqn\oft\\
\sqrt{\frac{GM\ofrteqn}{{\mathbf r}}} \sim {\mathbf r}^{0.2} & {\mathbf r} >{\mathbf r}_*(t),
\end{dcases}
...
\item The stellar mass increases quadratically with time. 
This result  arises naturally from the attractor solution nature of the density profile at small $\vec r$, Equation \ref{eq:hydro_density}, 
and the scaling with Keplerian velocity for the turbulent and infall velocities at small $\vec r$, Equation \ref{eq:hydro_infall_behavior}.
...
\dot M\ofrteqn=
\begin{dcases}
4\pi R^2\rho(R)\ureqn\ofrteqn, \sim t\,{\mathbf r}^{0} & \rrseqn\\
4\pi R^2\rho(R)\ureqn \ofrteqn \sim t^0\,{\mathbf r}^{0.2} & {\mathbf r} > {\mathbf r}_*.
\end{dcases}
...
MC15's predictions for $\rrs$ could not be checked using the simulations
...
both $\ur$ and $\vt$ exceed $c_s$, except at the earliest times ($\sim 100,000\yrs$ before a star forms), and then only for $\vec{r} \lesssim0.1\pc$, 
...
All the figures we show present results for stars with masses no larger than about $4 \Msun$. 
To estimate the effects of radiation, we compare the force from the Reynolds stress ${\mathbf F}_T=4\pi {\mathbf r}^2 \rho \vteqn ^2$, to the radiation force $L/c$. 
From Figure \ref{fig:hydro_velocity_avg}, the (averaged over many stars) $v_T$ is slightly in excess of $1\kms$ at $r=0.01\pc$, 
while from any of the density figures the density is $\rho\approx5\times10^{-18} {\rm g cm}^{-3}$. The force from Reynolds stress is then $F\approx4\times 10^{26}$ dynes. The luminosity of a 4 solar mass star on the zero age main sequence is $L\approx 2\times10^{36}\ergs$ \citep{1992A&AS...96..269S}, so the radiation force $L/c \approx 3\times10^{25}\,{\rm dynes}$, about a 10\% effect.  The force from Reynolds stress increases outward, see Figure \ref{fig:hydro_dPdr_avg}, so this statement holds at larger radii as well. 
